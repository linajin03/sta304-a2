...
To calculate the Confidence Interval for a proportion, the following formula will be used:
...
-   n is the sample size of Liberal voters

-   z is the critical value from the standard Normal Distribution corresponding to our desired confidence level (which in this case, is for a 95% confidence)
...
$$ where - $P(Vote Liberal)$ is the probability of voting for the Liberal party - $\beta_0$ is the intercept value - $\beta_1$ is the effect of ISCED Level (Education Level) on the log-odds of voting Liberal. - $\beta_2$ is the effect of Age on the log-odds of voting Liberal.
